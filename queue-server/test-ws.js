...
async function run() {
  ...
  if (!queueServer) {
    throw new Error('Queue-Server is not running on the candidate ports.');
  }

  const ws = new WebSocket(queueServer.wsUrl);

  ws.on('open', () => {
    console.log(`WS Connected on port ${queueServer.port}`);
    ws.send(JSON.stringify({ type: 'register', clientType: 'extension' }));
    setTimeout(() => {
      const req = http.request(`${queueServer.httpUrl}/tasks`, { method: 'POST', headers: { 'Content-Type': 'application/json' } }, (res) => {
        let data = '';
